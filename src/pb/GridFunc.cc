        ...
        for (int ii = 0; ii < ldim[0]; ii++)
            for (int jj = 0; jj < ldim[1]; jj++)
                memcpy(uu_ + (ii + ghosts_dst) * incx_
                    ...
                    src.uu_ + (ii + ghosts_src) * incx_src
    ...
    for (int ix = nghosts_fine; ix < cdim0 + nghosts_fine; ix++)
    {
        ...
        for (int iy = nghosts_fine; iy < cdim1 + nghosts_fine; iy++)
        {
            ...
            for (int iz = 0; iz < cdim2; iz++)
            {
                ...
                ucoarse.uu_[iyc + iz + nghosts_coarse] = (T)(
...
void GridFunc<T>::test_trade_boundaries()
{
    ...
    if (mype_env().my_mpi(0) == 0 && !bc_[0])
        initx = ghost_pt();
    else
        initx = 0;
    if (mype_env().my_mpi(1) == 0 && !bc_[1])
        inity = ghost_pt();
    else
        inity = 0;
    if (mype_env().my_mpi(2) == 0 && !bc_[2])
        initz = ghost_pt();
    else
        initz = 0;

    if (mype_env().my_mpi(0) == (mype_env().n_mpi_task(0) - 1))
        endx = ghost_pt() + dim(0) - 1;
    else
        endx = 2 * ghost_pt() + dim(0);
    if (mype_env().my_mpi(1) == (mype_env().n_mpi_task(1) - 1))
        endy = ghost_pt() + dim(1) - 1;
    else
        endy = 2 * ghost_pt() + dim(1);
    if (mype_env().my_mpi(2) == (mype_env().n_mpi_task(2) - 1))
        endz = ghost_pt() + dim(2) - 1;
    else
...
void GridFunc<T>::assign(const GridFunc<T>& src, const char dis)
{
    ...
    else // more expensive assign
    {
        ...
        const T* const u_src
            = src.uu_ + kstart + jstart * incy_src + istart * incx_src;
